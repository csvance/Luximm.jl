"""Dump input + reference outputs for a CoAtNet variant.

Writes a small HDF5 fixture per variant in `COATNET_VARIANTS_FULL`:

    /input                       deterministic torch.randn (seeded), NCHW
    /output/features             timm forward_features(x)
    /output/logits               timm forward(x); CoAtNet ships a head
    /state_dict/<no entries>     placeholder; weights load live from HF

Usage:
    uv run python test/parity/dump_coatnet_io.py --variant coatnet_0_rw_224.sw_in1k
    uv run python test/parity/dump_coatnet_io.py --all
"""
...
def main() -> None:
    ...
    parser.add_argument("--all", action="store_true",
                        help="Dump every variant in COATNET_VARIANTS_FULL.")
    ...
    if args.all:
        for short, full in COATNET_VARIANTS_FULL.items():
            run_one(short, full,
    ...
    arg = args.variant
    if arg in COATNET_VARIANTS_FULL:
        short, full = arg, COATNET_VARIANTS_FULL[arg]
    else:
        short = next((k for k, v in COATNET_VARIANTS_FULL.items() if v == arg), None)
        if short is None:
            parser.error(f"unknown variant: {arg}. "
                         f"Known short keys: {sorted(COATNET_VARIANTS_FULL.keys())}; "
                         f"known full names: {sorted(COATNET_VARIANTS_FULL.values())}")
        full = arg
